...
RAW_LANDSAT_DIR = BASE_DATA_DIR / "raw_landsat_LR"
RAW_ALPHA_DIR = BASE_DATA_DIR /"raw_alphaearth_HR"
PROCESSED_DATA_ROOT = BASE_DATA_DIR / "processed_data_try_2"
...
def main_processing():
    ...
    for landsat_path in tqdm(all_landsat_files, desc="Scanning dates for task planning"):
        ...
        sampled_coords = set()
        tries = 0
        max_tries = MAX_PATCHES * 100
        patches_found = 0
        while patches_found < MAX_PATCHES and tries < max_tries:
            r = random.randrange(0, proc_h - LR_PATCH_SIZE + 1, LR_PATCH_SIZE)
            c = random.randrange(0, proc_w - LR_PATCH_SIZE + 1, LR_PATCH_SIZE)
            coord = (r, c)
            if coord in sampled_coords:
                tries += 1
                continue
            if np.count_nonzero(combined_mask[r:r+LR_PATCH_SIZE, c:c+LR_PATCH_SIZE]) / (LR_PATCH_SIZE**2) >= VALID_PIXEL_RATIO_THRESHOLD:
                ...
                patches_found += 1
            tries += 1
        print(f"[INFO] Randomly sampled {patches_found} valid patches for {landsat_path.name}")
