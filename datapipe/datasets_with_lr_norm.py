...
class PreprocessedTileDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, **params):
        ...
        # 【新增】归一化配置参数
        self.normalize_percentile_lo = float(params.get('normalize_percentile_lo', 1.0))
        self.normalize_percentile_hi = float(params.get('normalize_percentile_hi', 99.0))
        self.lr_physical_norm_cfg = dict(params.get('lr_physical_norm', {}))
    ...
    def __getitem__(self, index):
        ...
        # 【改进】归一化统一在 Dataset：先物理归一化，再做分位拉伸
        lr_img = apply_landsat_lr_physical_normalization(lr_img, self.lr_physical_norm_cfg)
        lr_normalized = robust_per_image_normalize(
            lr_img,
            lo=self.normalize_percentile_lo,
            hi=self.normalize_percentile_hi,
        )
...
def apply_landsat_lr_physical_normalization(lr_img, cfg):
    """按配置对 Landsat LR 原始值做物理归一化。
    
    说明：
    - 该步骤用于把 prepare_data_local 中的归一化职责迁移到 Dataset。
    - 可通过 auto_detect_raw_input 自动跳过已归一化输入，避免重复处理。
    
    参数 cfg 示例：
    {
      'enabled': True,
      'reflective_band_indices': [0, 1, 2, 3, 4, 5, 6],
      'thermal_band_indices': [7, 8],
      'reflectance_scale': 1e-4,
      'thermal_offset': 250.0,
      'thermal_scale': 80.0,
      'clip_to_unit': True,
      'auto_detect_raw_input': True,
      'raw_input_threshold': 2.0
    }
    """
    out = lr_img.astype(np.float32, copy=True)

    if not bool(cfg.get('enabled', True)):
        return out

    reflective_bands = cfg.get('reflective_band_indices', [0, 1, 2, 3, 4, 5, 6])
    thermal_bands = cfg.get('thermal_band_indices', [7, 8])

    reflectance_scale = float(cfg.get('reflectance_scale', 1e-4))
    thermal_offset = float(cfg.get('thermal_offset', 250.0))
    thermal_scale = float(cfg.get('thermal_scale', 80.0))
    clip_to_unit = bool(cfg.get('clip_to_unit', True))

    auto_detect = bool(cfg.get('auto_detect_raw_input', True))
    raw_threshold = float(cfg.get('raw_input_threshold', 2.0))

    should_apply = True
    valid_reflective = [idx for idx in reflective_bands if 0 <= idx < out.shape[0]]
    if auto_detect and valid_reflective:
        probe = out[valid_reflective]
        should_apply = bool(np.nanpercentile(np.abs(probe), 95) > raw_threshold)

    if not should_apply:
        return out

    valid_thermal = [idx for idx in thermal_bands if 0 <= idx < out.shape[0]]

    if valid_reflective:
        out[valid_reflective] *= reflectance_scale
        if clip_to_unit:
            out[valid_reflective] = np.clip(out[valid_reflective], 0.0, 1.0)

    if valid_thermal:
        out[valid_thermal] *= reflectance_scale
        out[valid_thermal] = (out[valid_thermal] - thermal_offset) / thermal_scale
        if clip_to_unit:
            out[valid_thermal] = np.clip(out[valid_thermal], 0.0, 1.0)
...
class AnytimeTemporalDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, **params):
        ...
        self.target_hr_rule = str(params.get('target_hr_rule', 'latest')).lower()
        ...
        # --- 【新增】规范化配置参数 ---
        norm_cfg = parent_configs.get('normalization', {})
        self.normalize_percentile_lo = float(norm_cfg.get('percentile_lo', 1.0))
        self.normalize_percentile_hi = float(norm_cfg.get('percentile_hi', 99.0))
        self.lr_physical_norm_cfg = norm_cfg.get('lr_physical_norm', {})
        ...
        if self.enable_hr_zero_filter:
            ...
            for tile_id in self.tile_ids:
                if self._target_sample_num is not None and len(kept_tile_ids) >= self._target_sample_num:
                    break
                all_lr_files = sorted(self.lr_dir.glob(f"*_tile_{tile_id}.tif"))
    ...
    def __getitem__(self, index):
        ...
        # 加载HR：默认监督目标对齐“最新时相”，避免输入序列与监督日期错位
        target_rule = str(getattr(self, "target_hr_rule", "latest")).lower()
        if target_rule not in ("latest", "earliest"):
            target_rule = "latest"
        target_lr_file = lr_files[-1] if target_rule == "latest" else lr_files[0]
        target_hr_path = self.hr_dir / target_lr_file.name
        with rasterio.open(target_hr_path) as src:
            hr_img = src.read()
        hr_tensor = torch.from_numpy(robust_per_image_normalize(
            hr_img,
            lo=self.normalize_percentile_lo,
            hi=self.normalize_percentile_hi,
        )).float()
        ...
        for lr_path in lr_files:
            ...
            # --- 【新增】应用Landsat物理归一化 ---
            if self.lr_physical_norm_cfg:
                reflectance_data = apply_landsat_lr_physical_normalization(
                    reflectance_data, self.lr_physical_norm_cfg
                )
            ...
            # --- 【核心改造】根据开关，动态拼接特征 ---
            # 1. 基础特征：归一化后的反射率
            normalized_reflectance = robust_per_image_normalize(
                reflectance_data,
                lo=self.normalize_percentile_lo,
                hi=self.normalize_percentile_hi,
            )
        ...
        return _pad_or_truncate_temporal_sample(sample, self.fixed_temporal_len)
