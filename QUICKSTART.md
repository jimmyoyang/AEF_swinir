...
├── train.sh                          # 快速脚本：单配置训练
├── check_iterations.sh               # 查看配置中的 iterations 值
├── scripts/
│   ├── best_ckpt_manager.py          # 最佳权重管理器
│   ├── check_and_generate_configs.py # 配置检查器
...
只需一个命令，脚本自动修改所有 YAML 配置的 iterations：

### 快速测试（100迭代）
```bash
bash run_ablation_experiments_extended.sh
```
...
- **集中管理**：脚本统一控制所有 5 个模型的配置和 iterations
...
输出示例：
```
config_true_baseline.yaml            iterations = 1000
ablation_2b_with_cross_attention...  iterations = 1000
ablation_3a_with_cross_attention...  iterations = 1000
ablation_3b_with_cross_attention...  iterations = 1000
ablation_3c_with_cross_attention...  iterations = 1000
```
...
**对比链路**：建议 `2b → 4a → 4c`，验证软掩膜 + 聚合策略的叠加收益。
...
# 3) 重点查看 2b / 4a / 4c 三组对照
grep -E "ablation_2b|ablation_4a|ablation_4c" ablation_summary_latest.csv
```
